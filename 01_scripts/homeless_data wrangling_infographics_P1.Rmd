...
```
## Summarizing the tables by grades per regions

```{r, echo=TRUE, results='hide', message = FALSE, include=TRUE}


homeless.ny_regions <- readr::read_csv("00_data/regions_ny.csv") %>% 
  dplyr::unique()

homeless.ny_grades_regions <- readr::read_csv("00_data/00_homeless_student_grades.csv") %>%        dplyr::left_join(homeless.ny_regions) %>% 
  ...
  dplyr::mutate(DIFF = `2021`-`2009`,
                PCT_INCREASE = ((DIFF*100)/(`2009`))) %>% 
  dplyr::filter(PCT_INCREASE != 0) %>% 
  #tidyr::pivot_longer(cols = 2:3, names_to = "Year", values_to = "Indiv") %>% 
